...
$blockedUsersQuery = "SELECT u.id, u.username FROM blocked_users b 
                      JOIN userstable u ON b.blocked_id = u.id 
                      WHERE b.blocker_id = ?";
$stmt = $conn->prepare($blockedUsersQuery);
$stmt->bind_param("i", $user_id);
$stmt->execute();
$blockedUsersResult = $stmt->get_result();
$blockedUsers = $blockedUsersResult->fetch_all(MYSQLI_ASSOC);
?>
        ...
        <?php if (!empty($blockedUsers)): ?>
            <ul>
                <?php foreach ($blockedUsers as $blockedUser): ?>
                    <li>
                        <?php echo htmlspecialchars($blockedUser['username']); ?>
                        <button onclick="toggleBlock(<?php echo $blockedUser['id']; ?>)">Unblock</button>
                    </li>
                <?php endforeach; ?>
            </ul>
        <?php else: ?>
...
<script>
    ...
    let button = document.getElementById("block-btn");

    let action = button.innerText.includes("Block") ? "block_user.php" : "unblock_user.php";

    fetch(action, {
        method: "POST",
        body: new URLSearchParams({ blocked_id: userId }),
