...
$checkBlockQuery = "SELECT * FROM blocked_users WHERE (blocker_id = ? AND blocked_id = ?) OR (blocker_id = ? AND blocked_id = ?)";
$stmt = $conn->prepare($checkBlockQuery);
$stmt->bind_param("iiii", $senderId, $receiverId, $receiverId, $senderId);
$stmt->execute();
$result = $stmt->get_result();

// **If blocked, prevent fetching messages**
if ($result->num_rows > 0) {
    echo "<p class='error-message'>You cannot view messages from this user.</p>";
    exit();
}
